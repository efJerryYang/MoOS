...
use crate::{
    console::print,
    mm::{memory_set, page_table::copy_out, MemorySet},
    task::Thread,
};
use alloc::{
    boxed::Box,
...
use crate::{
    ...
    sbi::console_getchar,
    task::{
        global_buffer_list, global_dentry_cache, global_open_file_table, task_list, FdManager,
...
impl Thread {
    ...
    pub fn sys_mount(&self) -> isize {
        let block_device =
            ImageFileBlockDevice::new("path/to/sdcard.img").expect("Failed to open image file");

        let sdcard = SdCard::new(block_device).expect("Failed to initialize SD card");

        let time_source = MyTimeSource;

        let mut volume_manager = VolumeManager::new(sdcard, time_source);

        let volume = volume_manager.get_volume(embedded_sdmmc::VolumeIdx(0))?;

        let root_dir = volume_manager.open_root_dir(&volume)?;

        let mut file =
            volume_manager.open_file_in_dir(&volume, &root_dir, "MY_FILE.TXT", Mode::ReadOnly)?;

        let mut buffer = [0u8; 512];
        let num_read = volume_manager.read(&volume, &mut file, &mut buffer)?;

        for b in &buffer[0..num_read] {
            print!("{}", *b as char);
        }

        volume_manager.close_file(&volume, file)?;
        volume_manager.close_dir(&volume, root_dir)?;
