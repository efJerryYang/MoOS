...
pub unsafe fn clone(stack: usize) -> usize {
    ...
    tasks[pid].trapframe_ppn = tasks[pid]
        .memory_set
        .translate(VirtAddr::from(TRAPFRAME).into())
        .unwrap()
        .ppn();
    *(tasks[pid].trapframe_ppn.get_mut() as *mut TrapFrame) =
        *(tasks[nowpid].trapframe_ppn.get_mut() as *mut TrapFrame);
    (*(tasks[pid].trapframe_ppn.get_mut() as *mut TrapFrame)).x[10] = 0;
    (*(tasks[pid].trapframe_ppn.get_mut() as *mut TrapFrame)).kernel_sp =
...
pub unsafe fn sched() {
    ...
    __switch(
        &mut p.context as *mut ProcessContext,
        &mut mycpu().context as *mut ProcessContext,
    );
}
...
pub unsafe fn schedule() {
    ...
    loop {
        ...
        loop {
            ...
            idx += 1;
            if (idx >= task_list.exclusive_access().len()) {
                break;
            }
        }
    }
